<script lang="ts">
	...
	onMount(async () => {
		try {
			...
			// v2 manifest entries embed previewUrl directly (portraits-thumb/*.jpg).
			// v1-placeholder entries carry previewUrl via the raw `urls.preview` field.
			// Primary WB upstream: fall back to gradient swatch.
			const rawUrls = await _readPreviewUrls(source);
			pillars = arr.map((p) => ({
				...p,
	...
	async function _readPreviewUrls(
		src: PillarManifestSource | null,
	): Promise<Map<number, string>> {
		const map = new Map<number, string>();
		if (src === 'pillar-library-v2') {
			// v2 manifest embeds previewUrl on each entry — fetch raw to extract it.
			try {
				const res = await fetch('/pillar-library-v2/manifest.json');
				...
				for (const e of arr) {
					if (typeof e.pillarId === 'number' && e.previewUrl) {
						map.set(e.pillarId, e.previewUrl);
					}
				}
			} catch {
				/* no-op — tiles fall back to gradient swatch */
			}
			return map;
		}
		if (src !== 'placeholder') return map;
		// v1 placeholder: previewUrl lives in urls.preview
		try {
...
<section class="station">
	...
	<p class="note">
		...
		{:else if source === 'pillar-library-v2'}
			Pick one of 150 kid archetypes — portraits generated by AI, taxonomy
			population-weighted for global diversity.
